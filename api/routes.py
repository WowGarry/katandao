...
def create_game():
    """
    Initializes a new Catan session with randomized map and harbors.
    """
    try:
        data = request.json
        ...
        # Populate Players
        for i, p_info in enumerate(players_data[:player_count]):
            player = Player(
                player_id=i + 1,
                name=p_info.get('name', f'Player {i+1}'),
                color=p_info.get('color', 'blue'),
        ...
        if map_type == 'balanced':
            game_state.hex_map = MapGenerator.generate_balanced_map(seed)
        else:
        ...
        # Assuming GameLogger is defined elsewhere in your logic
